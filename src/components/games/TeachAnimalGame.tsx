 ...
 * asking HER to teach it. She records herself saying just that character;
 * the animal then "recites" the whole sentence again, this time using her
 * own recording spliced in for the word it learned — so the payoff is both
 * "I taught someone" and "I can hear myself as part of the sentence". */
export function TeachAnimalGame({ sentence, onComplete, onSkip }: Props) {
  ...
  useEffect(() => {
    ...
    readSeq.done.then(() => {
      ...
      const askSeq = speakSequence(["嗯…這個字我不會念，你可以教我嗎？"], ANIMAL_VOICE);
      askSeq.done.then(() => {
  ...
  return (
    <div>
      ...
      <p style={{ textAlign: "center", color: "var(--color-text-muted)", margin: "0 0 12px", minHeight: "1.4em" }}>
        ...
        {phase === "recording" && `牠卡住了！錄音教牠念「${missingChar}」吧`}
      ...
      <SentenceCard
        ...
        askingIndex={askingIndex}
        foundIndex={foundIndex}
      />
      {phase === "recording" && (
        <div style={{ textAlign: "center", marginTop: 12 }}>
          <RecordButton
            label={`🎤 教我念「${missingChar}」`}
