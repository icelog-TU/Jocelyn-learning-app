...
- **直式注音排版**：造句練習卡片裡的句子採用直式（由上到下、欄與欄之間由右到左）排版，注音標在每個字的右邊，跟一般注音課本、注音讀物的排法一樣，不是橫式文字上面加注音。句子太長時會自動分成好幾欄（每欄最多 6 個字）。逗號句號也做了對齊校正，避免因為全形標點符號本身在字型裡的位置偏高，而看起來「浮」在字的上方。二、三、四聲的聲調符號（ˊˇˋ）跟輕聲的小圓點（˙）也拆出來用比較小的字級顯示、貼在注音符號的右上角（輕聲則貼左上角），跟真正的注音讀本一樣是個小記號，而不是跟其他注音符號一樣大小、疊成一整排。
