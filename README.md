...
- **AI 造句練習（以句子為主的學習方式）**：按「產生新句子」會跳出對話框，輸入一個她已經學過的字（例如「學」）並選擇難度（簡單／中等／困難，難度越高句子越長、念對了星星也越多：2／3／5 顆），AI 就會用這個字加上她已經學過的其他字，生成 5 句都包含這個字、盡量組成真實詞彙的句子（例如「我去學校上學」「學生喜歡老師」「他很好學」），而不是把字硬塞進句子裡；後端也會檢查生成結果，過濾掉沒有包含目標字或長度不符的句子。這批句子會標記「衍生自「學」」跟難度存起來，之後每天都能重複練習，不用每次重新生成；換個字（例如「男」）再按一次，就是新的一批、分開存放。念完由旁邊的人（或她自己）按「我念對了」拿星星，念錯按「先跳過」，不做語音辨識判斷（不準確，容易誤判打擊信心）。她也可以按「錄音念念看」把自己念的錄下來，錄完立刻自己播放聽（只在當下播放，不會上傳或保存）。念對了會有星星動畫 + 音效 + 浮誇語音稱讚。句子生成不理想的話，可以在「管理句子庫」裡修改文字或刪除。這個功能需要額外設定，見下方「設定 AI 造句練習」。
- **學習紀錄**：分成兩個分頁 —「漢字紀錄」依日期列出每天新增了哪些字、來自哪本繪本；「句子紀錄」依日期列出每天生成/練習了哪幾句、每句是衍生自哪個字。
