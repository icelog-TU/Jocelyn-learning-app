...
- **AI 造句練習頁**（造句分頁）：**打開這一頁本身不會呼叫 AI**——如果句子庫已經有存句子，會直接從整個句子庫（所有日期、所有字詞累積起來的）裡挑最多 10 句到期該複習的句子讓你練習（類似記憶卡盒 Leitner box 的排程），這些都是本來就存在「管理句子庫」裡的舊句子，可以在那邊找到、修改。只有按下「產生新句子」才會真的呼叫 AI，自由挑一個已經學過的字或詞另外生成一批（一樣會先進到可編輯清單，確認後才存起來）。句子不理想的話，除了練習當下可以直接改，也可以在「管理句子庫」裡修改文字或刪除。這個功能需要額外設定，見下方「設定 AI 造句練習」。
